...
to.train<-betaAdj[rownames(betaAdj) %in% proms,colnames(betaAdj) %in% training.set$Name]##adjust cellcomp
to.train<-betaAdj[rownames(betaAdj) %in% names(sel),colnames(betaAdj) %in% training.set$Name]##adjust cellcomp

sel.imp<-var.sel.boruta(t(to.train),y,pValue = 0.01, maxRuns = 100,ntree = 999, mtry.prop = 0.2, nodesize.prop = 0.1,no.threads = 8, method = "ranger", type = "regression")

###for perm change y to numeric

#perm.y<-ifelse(y=="R",1,0)

##sel.imp<-var.sel.perm(t(to.train),perm.y, no.perm  = 100,ntree = 999, mtry.prop = 0.2, nodesize.prop = 0.1,no.threads = 8, method = "ranger", type = "regression",parametric = F)
...
rf.pred <- randomForest(
  t(to.train[sel.imp$var,]),
 # t(to.train),
  y,
  #mc=8,
  ntree=ntrees,
#  strata=y,
  mtry=sqrt(ncol(t(to.train[sel.imp$var,]))),
  #mtry=100,
...
ntrees <- 999
#p <- 150 ##select 1st 1000 with High Gini Scores
y <-as.factor(training.set[,"RESP"])
...
for(K in 1:folds){
  ...
  for(k in 0:folds){
    ...
    rf.scores <- calcultateCVfold(t(to.train),y,fold,cores,ntrees)###original
...
##DMP Annotation
dmp <- merge(rf.pred$importance,ann850k[,c("UCSC_RefGene_Name","Relation_to_Island","UCSC_RefGene_Group")], by="row.names")
