...
# 2018-03-14 UTC
#------------------------------------------------------------------------------------                   
source("../../Classes_RNAseq_2019/Predictor_scripts/calculateCVfold_Boruta.R")
...
for(K in 1:folds){
  ...
  for(k in 0:folds){
    ...
    rf.scores <- calcultateCVfold_Boruta(t(to.train),y,fold,cores,ntrees)###original
...
save(cv.calfit,file="calfit_FIS.RData")

save(rf.pred,scores,probs,y,ys,yp,file="CVresults_Pred_FIS.RData")
...
out
write.csv(out, "results_Pred_FIS.csv")
...
dev.off()
save(predictions,file="Predictions_FIS_Boruta.RData")
#  ROC curve,
library(plotROC)
