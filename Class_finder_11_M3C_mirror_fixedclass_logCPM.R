...
setwd("/mnt/md127/Analysis_Projects/Classes_RNAseq_2019/SABG/")
...
source("Function_classes_fixed.R")
...
####Corticoids
  ...
  cortic<-cortic[cortic$indivId %in% c(A.set1,B.hi),]
  cortic<-cortic[match(samples2test$X,cortic$indivId),]
  
  design <- model.matrix(~group+samples2test$Sex + my.batch  + samples2test$age + cortic$IMID_Corticoids)#
...
#
 ...
 # corrected_vst_df <- removeBatchEffect(my.rpkm,design=treatment.design,covariates=batch.design)
...
#  my.selection<-c("SCL_logCPM")
  ...
  cortic<-readRDS("/media/IMID/Projects/Project4-Celgene/Data-Storage/Phenotype-Data/All_RNA_samples/As_predictors/PHENODATA_PRED.rds")
  cortic<-cortic[,c("indivId","IMID","IMID_Corticoids")]
  cortic<-cortic[cortic$indivId %in% c(A.set2,B.lo),]
  cortic<-cortic[match(samples2test$X,cortic$indivId),]
  
  design <- model.matrix(~group+samples2test$Sex + my.batch + samples2test$age)##monocytes error, out due to next
  rownames(design)<-samples2test$X
