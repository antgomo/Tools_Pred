...
week0<-read.csv("Week0_PACTABA.csv")
week12<-read.csv("Week12_PACTABA.csv")
...
keep <- rowSums(cpm(y)>2) >= 10#### change group depending replicates
y <- y[keep, ]
...
out
write.csv(out, "results_Pred_PACTABA_w0_48.csv")
...
plot(roc.mock, col = "red",main="PACTABA Gvs NR Week 0 EULAR w0_w48")
text(0.9,0.8,paste("AUC = ",format(roc.mock$auc, digits=3, scientific=FALSE)))
